...
def plot_phase_space_diff_conditions(p0_arr):
    ...
    plt.savefig(
        'results/phase_portraits_pairs.png',
        dpi=400,
        bbox_inches='tight'
    )
...
def noisy_volterra(std, p0, Num_steps, t_max):
    ...
    noise = (
        2 * torch.rand(Num_steps, 2).to(device) - 1
    ) * std
...
fig.savefig(
    'results/predator_predictions.png',
    dpi=400,
    bbox_inches='tight'
)
...
phase_residual = np.sqrt(
    residuals_prey**2 + residuals_pred**2
)
