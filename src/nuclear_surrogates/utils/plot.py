...
def plot_losses(train_losses, val_losses, save_dir, nfes=None):
    ...
    ax_loss.plot(
        log_train_losses, label="Training Loss (log10)", linewidth=2, color="#2E86AB"
    )
    ax_loss.plot(
        log_val_losses, label="Validation Loss (log10)", linewidth=2, color="#A23B72"
    )
    ...
    if nfes and len(nfes) == len(train_losses):
        ...
        ax_nfe.plot(
            nfes, label="NFE", linewidth=1.5, color="#FF9800", linestyle="--", alpha=0.8
        )
        ...
        ax_loss.legend(
            lines_loss + lines_nfe,
            labels_loss + labels_nfe,
            fontsize=10,
            loc="upper right",
        )
    else:
...
def plot_residuals_combined(actuals, predictions, plots_folder, steps_per_run):
    ...
    if np.sum(mask_combined) > 0:
        ...
        pred_range = [
            abs_delta_preds[mask_combined].min(),
    ...
    plt.close()
    logger.info(
        f"Concentration-change residuals (log-log scale) saved to: {filepath_log}"
    )
...
def plot_prediction_comparison(
    ...
    mare_teacher = np.mean(
        np.abs(ground_truth - teacher_forced_preds) / np.max(ground_truth)
    )
    mare_autoregressive = np.mean(
        np.abs(ground_truth - autoregressive_preds) / np.max(ground_truth)
    )
    ...
    time_steps = (
        np.arange(len(ground_truth)) + 1
    )  # Shift them by one to the right as t=0 isnt predicted
    ...
    ax1.plot(time_steps, teacher_forced_preds, "b-", linewidth=2, alpha=0.8, zorder=2)
    ax1.plot(
        time_steps,
        ...
        markeredgewidth=0.5,
        label=f"Teacher-Forced (MARE: {mare_teacher:.4f})",
        zorder=3,
    )

    # Autoregressive with triangle markers
    ax1.plot(time_steps, autoregressive_preds, "r-", linewidth=2, alpha=0.8, zorder=2)
    ax1.plot(
        time_steps,
        ...
        markeredgecolor="white",
        markeredgewidth=0.5,
        label=f"Autoregressive (MARE: {mare_autoregressive:.4f})",
        zorder=3,
    )
    ...
    ax2.plot(
        time_steps, residuals_autoregressive, "r-", linewidth=1.8, alpha=0.8, zorder=2
    )
    ax2.plot(
        time_steps,
        residuals_autoregressive,
        "^",
        color="red",
        markersize=6,
        markerfacecolor="red",
