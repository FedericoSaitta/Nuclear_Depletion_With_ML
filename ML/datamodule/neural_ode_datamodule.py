...
class NODE_Datamodule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        input_data_arr, self.col_index_map = data_help.split_df(
            data_df, self.inputs.keys()
        )
        target_data_arr, self.target_index_map = data_help.split_df(
        ...
        assert (
            num_runs * self.steps_per_run == num_total
        ...
        logger.info(
            f"Dropped {num_runs} NaN rows, {input_data_arr.shape[0]} samples remain"
        )
        assert not np.isnan(
            input_data_arr
        ...
        input_trajs = input_data_arr.reshape(
            num_runs, self.actual_steps, n_input_features
        )
        target_trajs = target_data_arr.reshape(
            num_runs, self.actual_steps, n_target_features
        )

        logger.info(f"Total trajectories: {num_runs}, each {self.actual_steps} steps")
        logger.info(
            f"Input features: {n_input_features}, Target features: {n_target_features}"
        )
        ...
        if self.inference_mode:
            ...
            # ══════════════════════════════════════════════════════════════
            logger.info(
                "Inference mode: scalers fitted on training data, loading inference data..."
            )

            inf_df, inf_steps_per_run, inf_time_array = data_help.read_data(
                self.path_to_inference_data,
                1.0,
                drop_run_label=True,
            )
            ...
            assert (
                inf_num_runs * inf_steps_per_run == inf_total
            ...
            inf_input_scaled = self.input_scaler.transform(inf_input_arr).reshape(
                inf_num_runs, inf_actual_steps, n_input_features
            )

            inf_target_scaled = self.target_scaler.transform(inf_target_arr).reshape(
                inf_num_runs, inf_actual_steps, n_target_features
            )
            ...
            dummy = torch.zeros(
                0, inf_actual_steps, n_input_features + n_target_features
            )
            ...
            logger.info(
                f"Training time span: {training_T:.4f}, Inference time span: {raw_t_inf[-1] - raw_t_inf[0]:.4f}"
            )
