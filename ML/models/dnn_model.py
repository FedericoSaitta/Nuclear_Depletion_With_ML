...
class DNN_Model(L.LightningModule):
    def __init__(self, config_object):
        ...
        self.residual_map = [
            (in_size == out_size)
            for in_size, out_size in zip(self.NN_layers[:-1], self.NN_layers[1:])
        ]
        if self.residual_connections and not any(self.residual_map):
            logger.error(
                "Residual connections requested, but no adjacent layers have matching input/output dimensions."
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        )  # doesnt get logged in sql databse on purpose
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_hat_unscaled = data_scaler.inverse_transformer(
            target_scaler, y_hat.detach().cpu().numpy()
        )
    ...
    def _process_single_output(
        ...
        plot.plot_predictions_vs_actuals(
            y_true, y_pred, mae_output, rmse_output, r2_output, output_dir
        )
        plot.plot_residuals_combined(
            y_true, y_pred, output_dir, steps_per_run=steps_per_run
        )
    ...
    def _compute_mare_comparison(
        ...
        for idx, target_name in enumerate(target_names):
            ...
            Y_test_original = data_scaler.inverse_transformer(
                datamodule.target_scaler, Y_test
            )
            tf_gt_raw = (
                Y_test_original[:, idx] if Y_test_original.ndim > 1 else Y_test_original
            )
            ...
            if datamodule.delta_conc:
                ...
                for run in range(n_runs):
                    ...
                    X_first_unscaled = data_scaler.inverse_transformer(
                        datamodule.input_scaler, X_test[run_start].reshape(1, -1)
                    )[0]

                    if target_name in datamodule.col_index_map:
                        initial_conc = X_first_unscaled[
                            datamodule.col_index_map[target_name]
                        ]
    ...
    def _compute_and_plot_importance(
        self, loader, output_idx, feature_names, output_dir, metric_config, metric_name
    ):
        """Helper to compute and plot a single importance metric."""
        logger.info(f"  Computing {metric_name}-based feature importance...")
        importance_means, importance_stds, baseline = (
            metrics.calculate_feature_importance(
                self.model,
                loader,
                self.device,
                n_repeats=5,
        ...
        for idx, target_name in enumerate(target_names):
            ...
            if delta_conc:
                ...
                if target_name in datamodule.col_index_map:
                    ...
                    autoregressive_preds = initial_conc + np.cumsum(
                        autoregressive_deltas
                    )
                    teacher_forced_preds = initial_conc + np.cumsum(
                        teacher_forced_deltas
                    )
                else:
                    logger.warning(
    ...
    def _plot_error_growth(
        ...
        for idx, target_name in enumerate(target_names):
            ...
            for run_idx in range(num_runs):
                ...
                if delta_conc:
                    X_first_unscaled = data_scaler.inverse_transformer(
                        datamodule.input_scaler, X_test[start_idx].reshape(1, -1)
                    )[0]
                ...
                epsilon = 1e-20
                tf_male = np.abs(
                    np.log10(np.abs(teacher_forced_preds) + epsilon)
                    - np.log10(np.abs(ground_truth) + epsilon)
                )
                ar_male = np.abs(
                    np.log10(np.abs(autoregressive_preds) + epsilon)
                    - np.log10(np.abs(ground_truth) + epsilon)
                )
